...
// Helper: parse blood pressure string into systolic and diastolic numbers
...
// Blood Pressure Risk scoring as per spec
...
function temperatureRisk(temp) {
  ...
  else return 0; // For any other unexpected cases
}
...
// Validate if any key data is missing or invalid
function hasDataQualityIssues(patient) {
  ...
  // Validate BP
  if (!bp || !parseBloodPressure(bp)) return true;
...
// Fetch patients with retries (handle rate limiting and errors)
...
async function main() {
  ...
  for (const patient of allPatients) {
    ...
    if (hasDataQualityIssues(patient)) {
      dataQualityIssues.add(patient.patient_id);
      continue; // skip scoring if data quality is bad
    }
    ...
    if (totalRisk >= 4) {
      highRiskPatients.add(patient.patient_id);
    }

    // Fever check as per temperature >= 99.6°F
    // Use numeric temperature for comparison
    let tempValue = patient.temperature;
    if (typeof tempValue === 'string') {
      tempValue = Number(tempValue.trim());
    }
    if (!isNaN(tempValue) && tempValue >= 99.6) {
      feverPatients.add(patient.patient_id);
    }
  }
